...
function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Refresh first so the middleware sees the new session cookie,
    // then navigate — otherwise Next.js can redirect back to /login.
    router.refresh();
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-3xl border border-blue-500/20 p-8 sm:p-10 shadow-2xl shadow-blue-500/5">
          ...
          <div className="flex flex-col items-center gap-3 mb-8">
            <div className="w-12 h-12 rounded-2xl bg-blue-500/10 flex items-center justify-center border border-blue-500/20">
              <LogIn className="w-5 h-5 text-blue-400" strokeWidth={2} />
            </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-slate-500 shrink-0">أو سجّل عبر البريد الإلكتروني</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <p className="text-center text-sm text-slate-500">
              ليس لديك حساب؟{" "}
              <Link href="/register" className="text-blue-400 font-medium hover:text-blue-300 hover:underline">إنشاء حساب مجاني</Link>
            </p>
          </form>
        </div>
      </div>
